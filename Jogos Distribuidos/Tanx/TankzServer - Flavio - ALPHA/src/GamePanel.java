...
public class GamePanel extends JPanel {
	...
	public int getAvatarIndex( String pid )
	{
		int rv = -1;
		synchronized( avatars )
		{
			...
			{
				// IMPORTANTE: string é objeto, que se compara com método. Ou seja, == não funciona!
				if( avatars.get(i).playerId().equalsIgnoreCase( pid ) ) {
					rv = i;
					break;
				}
			}
	...
	public void paint(Graphics g)
	{
		...
		if (avatars.size() > 1) {	// Is there anyone else?
			...
			for (int i = 0; i < avatars.size(); i++) {
				...
				{
					DatagramPacket deathPacket = PacketFactory.newShoot(player.x(), player.y(), Bullet.BULLET_SPEED, 0, player.playerId());
