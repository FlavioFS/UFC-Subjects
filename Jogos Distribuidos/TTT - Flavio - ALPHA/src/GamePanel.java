	...
	public Avatar getAvatar( String pid )
	{
		synchronized( avatars )
		{
			for( Avatar t : avatars )
			{
				...
				if( t.getPlayerId().equalsIgnoreCase( pid ) )
					return t;
			}
...
//		}
	...
	public void updateAvatarPositions () {
		double dt = GamePanel.getInstance().getDeltaTime();
		
		for (Avatar player : avatars) {
			player.moveToXY( player.speed.getX() * dt, player.speed.getY() * dt );
		}
	}
	...
	public void paint(Graphics g)
	{
		...
		synchronized( avatars )
		{
			...
			for( int i=0; i<avatars.size(); i++ )
			{
				...
				// Removes expired bombs and renders the others
				int bombRadius = (int) Bomb.BOMB_RADIUS_DEFAULT;
				ArrayList<Bomb> bombList = a.getBombList();
				
				for (int j = 0; j < bombList.size(); j++) {
					Bomb b = bombList.get(j);
					
					// Expired bombs are removed
					if (b.expired())
					{
						bombList.remove(j);
						continue;
					...
					int ipx = (int) pos.getX();
					int ipy = (int) pos.getY();
		...
		if (avatars.size() > 1) {	// Is there anyone else?
			...
			for (int i = 0; i < avatars.size(); i++) {
				ArrayList<Bomb> iBombList = avatars.get(i).getBombList();
				
				if (player.isBeingHit(iBombList))
				{
					DatagramPacket deathPacket = PacketFactory.newDied(player.getX(), player.getY());
					PacketUtils.injectPlayerId(deathPacket.getData(), player.getPlayerId());
					protocol.send(deathPacket);
					killAvatar(player.getPlayerId());
				}
			}
		...
		// Bomb drop
		if( LocalGameInput.getInstance().getState(KeyEvent.VK_SPACE) && player.isBombAvailable() && player.isAlive() )
		{
			double sx = player.getX();
			double sy = player.getY();
			
			player.dropBomb(sx, sy, player.getPlayerId());
			DatagramPacket bombPacket = PacketFactory.newBomb(sx, sy);
			PacketUtils.injectPlayerId(bombPacket.getData(), player.getPlayerId());
	...
	protected void removeDisconnectedAvatars()
	{
		...
		for( Avatar a : avatars )
		{
			if( !a.isConnected() )
			{
				System.out.println("[DISCONNECTED] " + a.getPlayerId() );
				graveyard.add( a );
			}
		}
